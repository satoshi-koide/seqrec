...
def data_collator(features):
    """
    Datasetから返された辞書のリストをバッチにまとめる。
    'target_id' を 'labels' にリネームする重要なお仕事。
    """
    batch = {}
    ...
    # 必須: labels (Trainerは 'labels' という引数名で正解を探す)
    if 'target_id' in features[0]:
        batch['labels'] = torch.stack([f['target_id'] for f in features])
    
    # オプション: 画像特徴量など
    if 'seq_img_feat' in features[0]:
        batch['image_feats'] = torch.stack([f['seq_img_feat'] for f in features])
        batch['target_img_feats'] = torch.stack([f['target_img_feat'] for f in features])
    if 'input_ratings' in features[0]:
        batch['input_ratings'] = torch.stack([f['input_ratings'] for f in features])
